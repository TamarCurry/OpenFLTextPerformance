...
class TextField extends InteractiveObject {
	...
	@:noCompletion private var __appendIndex:Int;
	...
	public function appendText (text:String):Void {
		...
		__appendIndex = __text.length;
		__updateText (__text + text, false);
	...
	@:noCompletion private override function __renderCanvas (renderer:CanvasRenderer):Void {
		...
		if (renderer.__isDOM && !__renderedOnCanvasWhileOnDOM) {
			...
			if (__isHTML) {
				...
				__updateText (HTMLParser.parse (__text, __textFormat, __textEngine.textFormatRanges), false);
		...
		if (__cacheBitmap != null && !__isCacheBitmapRender) {
			...
			if (__renderedOnCanvasWhileOnDOM) {
				...
				if (__isHTML && __rawHtmlText != null) {
					...
					__updateText (__rawHtmlText, false);
					__dirty = true;
					__layoutDirty = true;
					__setRenderDirty ();
	...
	@:noCompletion private function __replaceText (beginIndex:Int, endIndex:Int, newText:String, restrict:Bool):Void {
		...
		__updateText (__text.substring (0, beginIndex) + newText + __text.substring (endIndex), true);
		if (endIndex > __text.length) endIndex = __text.length;
	...
	@:noCompletion private function __updateText (value:String, clearTextEngineCache:Bool):Void {
		...
		// if the change to the text is too drastic,
		// clear the text engine's character size cache
		if ( clearTextEngineCache ) {
			__textEngine.clearSizeCache();
		}
	...
	@:noCompletion private function set_htmlText (value:String):String {
		...
		if (DisplayObject.__supportDOM) {
			...
			if (__renderedOnCanvasWhileOnDOM) {
				...
				__updateText (value, true);
				
			} else {
				
				range.end = __rawHtmlText.length;
				__updateText (__rawHtmlText, true);
