...
class TextEngine {
	...
	@:noCompletion private var __sizes:Array<Float> = [];
	@:noCompletion private var __useCachedSizes:Bool = false;
	...
	private function clearSizeCache():Void
	{
		if ( __sizes.length > 0 ) {
			__sizes = [];
		}
	...
	private function getLayoutGroups ():Void {
		...
		var lineFormat = null;
			...
			if (__useIntAdvances) {
				...
				for (i in startIndex...endIndex) {
					...
					width = __useCachedSizes ? __sizes[i] : __context.measureText (text.substring (startIndex, i + 1)).width;
		...
		// if we only have one format for the whole text field,
		// then we can cache the size of each character in the text.
		// This way, we aren't calling __context.measureText(...) more than we need to.
		// This is only useful if we've been using textfield.appendText(...)
		__useCachedSizes = textFormatRanges.length == 1;
		if ( __useCachedSizes ) {
			var textLength = text.length;
			var nw:Float = 0;
			var tw:Float = 0;
			var si:Float = 0;
			var start:Int = __sizes.length; // pick up where we left off
			for ( i in start...textLength ) {
				if (i < textLength - 1) {
					
					// Advance can be less for certain letter combinations, e.g. 'Yo' vs. 'Do'
					nw = __context.measureText (text.charAt (i + 1)).width;
					tw = __context.measureText (text.substr (i,  2)).width;
					si = tw - nw;
					
				} else {
					
					si = __context.measureText (text.charAt (i)).width;
